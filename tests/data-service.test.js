...
test('shows today and tomorrow only for two-day injection view', () => {
  // 요청서가 주·야·명일주간 구조라 모레 이후 계획은 신뢰할 수 없어 2일 창만 노출.
  assert.deepEqual(
    DataService.getVisibleWeekdays(['월', '화', '수'], '화', '2days'),
    ['화', '수']
  );
  // 끝 원소면 내일 없이 오늘만 — end-clamp.
  assert.deepEqual(
    DataService.getVisibleWeekdays(['월', '화'], '화', '2days'),
    ['화']
  );
});
...
test('buildInkPlanningAlerts: 실데이터 합성 — 사출 수요+수동 재고로 부족·소진 동시 산출', () => {
  const data = {
    ...
    inkPlan: [{ name: 'INK1', days: { 월: { 현재고: 2 } } }],
    testInks: [],
    inventory: null,
  };
  const alerts = DataService.buildInkPlanningAlerts(data, {}, '월');
  // 오늘+내일 부족: 월 현재고 2 − (월2+화1=3) = −1 (모레 이후·차주월은 신뢰 불가라 제외)
