...
// ── inventory: parsePastedStockColumn (엑셀 세로 열 붙여넣기) ──────────────────
test('parsePastedStockColumn: 줄바꿈 분리 · 숫자만 num · 끝 빈 줄 제거', () => {
  const r = DataService.parsePastedStockColumn('10\n0\n\n25\n');   // 중간 빈칸 보존, 끝 개행 제거
  assert.equal(r.length, 4);
  assert.deepEqual(r.map(x => x.num), [10, 0, null, 25]);
});

test('parsePastedStockColumn: 콤마·공백·CRLF·탭(첫 칸만) 처리', () => {
  const r = DataService.parsePastedStockColumn('1,000\r\n  42  \r\n7\t9');
  assert.deepEqual(r.map(x => x.num), [1000, 42, 7]);   // 1,000→1000, 트림, 탭 첫 칸(7)
});

test('parsePastedStockColumn: 비숫자·빈 입력 안전', () => {
  assert.deepEqual(DataService.parsePastedStockColumn('abc').map(x => x.num), [null]);
  assert.deepEqual(DataService.parsePastedStockColumn('').map(x => x.num), [null]);
  assert.deepEqual(DataService.parsePastedStockColumn(null), [{ raw: '', num: null }]);
});
...
test('matchOcrRow: 설비 정지 표기 → skip + isStop (TEST와 동일 스킵, 라벨만 정지)', () => {
  // 표기 변형: 대소문자/한글/구분자·공백
  for (const name of ['stop', 'STOP', '정지', '호기 정지', '설비정지', '멈춤', '중단']) {
    const r = DataService.matchOcrRow({ product_name: name }, MASTER);
    assert.equal(r.status, 'skip', `${name} → skip이어야 함`);
    assert.equal(r.isStop, true, `${name} → isStop이어야 함`);
    assert.equal(r.isTest, true, `${name} → isTest도 true(동일 스킵 경로)`);
  }
  // 구분(brand)란이 정지여도 인식
  const byBrand = DataService.matchOcrRow({ product_name: '3호기', brand: 'STOP' }, MASTER);
  assert.equal(byBrand.isStop, true);
  // 반례: 'STOP'이 부분 문자열인 실제품명은 정지 아님(정확 일치만)
  const nonStop = DataService.matchOcrRow({ product_name: 'STOPPER RED', brand: '갑' }, MASTER);
  assert.equal(nonStop.isStop || false, false);
  // TEST 행은 isStop 아님(구분됨)
  assert.equal(DataService.matchOcrRow({ product_name: 'TEST' }, MASTER).isStop, false);
});
...
// ── validateInkForLot: LOT 번호↔잉크 이름 정합 검증 (순수 함수) ──────────────────
test('validateInkForLot: 일치하면 ok true, reason null', () => {
  const r = DataService.validateInkForLot('SHAD051301', 'SHADOW');
  assert.equal(r.ok, true);
  assert.equal(r.reason, null);
});

test('validateInkForLot: prefix 불일치 → prefix-mismatch', () => {
  // lotPrefix('SHADOW') === 'SHAD' → 'XYZ051301' 는 'SHAD' 로 시작하지 않음
  const r = DataService.validateInkForLot('XYZ051301', 'SHADOW');
  assert.equal(r.ok, false);
  assert.equal(r.reason, 'prefix-mismatch');
});

test('validateInkForLot: 잉크명 비어있으면 no-name', () => {
  assert.deepEqual(DataService.validateInkForLot('SHAD051301', ''), { ok: false, reason: 'no-name' });
  assert.deepEqual(DataService.validateInkForLot('SHAD051301', '   '), { ok: false, reason: 'no-name' });
});

test('validateInkForLot: 한글 이름은 prefix 추출 불가 → no-prefix', () => {
  // lotPrefix('잉크마스터') === '' (A-Z0-9만 남기므로)
  const r = DataService.validateInkForLot('SHAD051301', '잉크마스터');
  assert.equal(r.ok, false);
  assert.equal(r.reason, 'no-prefix');
});

// ── addInkToMaster: 잉크 마스터 추가 (순수 함수, 입력 미변경) ─────────────────────
test('addInkToMaster: machineAssignments[0].ink·inkPlan prepend, 원본 배열 미변경', () => {
  const data = {
    machineAssignments: [{ ink: 'OLDINK', machine: '1ho', code: 'C-1' }],
    inkPlan: [{ name: 'OLDINK', days: { 월: { '현재고': 1 } } }],
    products: [],
  };
  const beforeAssignments = JSON.parse(JSON.stringify(data.machineAssignments));
  const beforeInkPlan = JSON.parse(JSON.stringify(data.inkPlan));

  const next = DataService.addInkToMaster(data, { name: 'NEWINK', code: 'C-100', machine: '3ho' });

  // prepend 확인
  assert.equal(next.machineAssignments[0].ink, 'NEWINK');
  assert.equal(next.machineAssignments[0].machine, '3ho');
  assert.equal(next.machineAssignments[0].code, 'C-100');
  assert.equal(next.inkPlan[0].name, 'NEWINK');
  // 기존 항목은 그대로 뒤로 밀림
  assert.equal(next.machineAssignments[1].ink, 'OLDINK');
  assert.equal(next.inkPlan[1].name, 'OLDINK');

  // 원본 입력 미변경 (deep equal)
  assert.deepEqual(data.machineAssignments, beforeAssignments);
  assert.deepEqual(data.inkPlan, beforeInkPlan);

  // blank days 는 WEEKDAYS 키를 모두 가짐
  const dayKeys = Object.keys(next.inkPlan[0].days);
  for (const d of DataService.WEEKDAYS) assert.ok(dayKeys.includes(d), `요일 ${d} 누락`);
});

test('addInkToMaster: 이미 같은 정규화 이름이 inkPlan에 있으면 inkPlan은 건드리지 않음', () => {
  const data = {
    machineAssignments: [],
    inkPlan: [{ name: 'NEWINK', days: {} }],
    products: [],
  };
  const next = DataService.addInkToMaster(data, { name: 'NEWINK', code: 'C-100', machine: '3ho' });
  // machineAssignments엔 prepend 되지만
  assert.equal(next.machineAssignments[0].ink, 'NEWINK');
