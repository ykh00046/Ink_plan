...
test('excel import picks the sheet with the latest date columns', () => {
  // 차트시트(에러) → 옛 시트(2월) → 최신 시트(7월): 최신 시트 index 반환
  const list = [
    { error: 'no-header' },
    { dateCols: [{ label: '2/3' }, { label: '2/7' }], rows: [{}] },
    { dateCols: [{ label: '7/16' }], rows: [{}] },
  ];
  assert.equal(DataService.pickInventorySheetIndex(list), 2);

  // 파싱 가능한 시트가 하나도 없으면 -1
  assert.equal(
    DataService.pickInventorySheetIndex([
      { error: 'no-header' },
      { dateCols: [], rows: [] },
      { dateCols: [{ label: '7/16' }], rows: [] },
    ]),
  ...
  // 날짜 라벨이 없더라도 파싱 가능한(행이 있는) 시트는 선택된다
  const noDateList = [{ error: 'no-header' }, { dateCols: [], rows: [{ ink: 'X' }] }];
  assert.equal(DataService.pickInventorySheetIndex(noDateList), 1);

  // null/빈 입력도 안전하게 -1
  assert.equal(DataService.pickInventorySheetIndex(null), -1);
  assert.equal(DataService.pickInventorySheetIndex([]), -1);
});
